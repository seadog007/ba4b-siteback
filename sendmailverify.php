...
if(preg_match("/[a-zA-Z0-9]{1,12}/",$name)&&preg_match("/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z]+$/",$email)){
    ...
    $sql = "SELECT `BAHA_HASH`,`verifycomplete` FROM `verify` WHERE `BAHA_ID`='" . mysqli_real_escape_string($con,$name) . "' order by `ID` desc LIMIT 1";
    $result = @mysqli_query($con, $sql);
    $data = $result->fetch_array();
    if($data[0]==$hash&&$data[1]==1){
           $sql = "INSERT INTO `emailverify` (`ID`, `BAHA_ID`, `EMAIL`, `EMAIL_HASH`, `TIME`, `EXPIRE_TIME`, `IP`, `verifycomplete`) VALUES (NULL, '" . mysqli_real_escape_string($con,$name) . "', '" . mysqli_real_escape_string($con,$email) . "', '" . mysqli_real_escape_string($con,$Ehash) . "', '" . mysqli_real_escape_string($con,$Time) . "', '" . mysqli_real_escape_string($con,$Exp) . "', '" . mysqli_real_escape_string($con,$IP) . "', '0');";
