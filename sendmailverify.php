...
if(preg_match("/[a-zA-Z0-9]{1,12}/",$name)&&preg_match("/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z]+$/",$email)){
    ...
    if($data[0]==$hash&&$data[1]==1){
        ...
        mysqli_close($con);
        if(pSendMail($email,"BA4B服務驗證信", getbahaname($name) . "您好，這裡是BA4B服務中心，<br>請進入網址：<br>" . SYS_URL . "verify.php?mode=email&id=" . $name . "&email=" . $email . "&hash=" . $Ehash . "<br>來完成驗證…<br>驗證網址有效期限為兩小時，<br>如發現頁面過期即有可能是超過驗證有效期限。<br>BA4B工作團隊 各種感謝你XD<br>（備註：如果你沒有申請ba4b服務，請不要回復此信件，請直接無視。）") == 1) {
