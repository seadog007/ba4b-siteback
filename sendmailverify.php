...
if(preg_match("/[a-zA-Z0-9]{1,12}/",$name)&&preg_match("/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z]+$/",$email)){
    ...
    $sql = "SELECT `BAHA_HASH`,`verifycomplete` FROM `verify` WHERE `BAHA_ID`='" . mysqli::escape_string($name) . "' order by `ID` desc LIMIT 1";
    $result = @mysqli_query($con, $sql);
    $data = $result->fetch_array();
    if($data[0]==$hash&&$data[1]==1){
           $sql = "INSERT INTO `emailverify` (`ID`, `BAHA_ID`, `EMAIL`, `EMAIL_HASH`, `TIME`, `EXPIRE_TIME`, `IP`, `verifycomplete`) VALUES (NULL, '" . mysqli::escape_string($name) . "', '" . mysqli::escape_string($email) . "', '" . mysqli::escape_string($Ehash) . "', '" . mysqli::escape_string($Time) . "', '" . mysqli::escape_string($Exp) . "', '" . mysqli::escape_string($IP) . "', '0');";
