...
function updatamail($id,$email,$ip)
{
    ...
    $sql = "UPDATE `list` SET  `BAHA_NAME` = '" . mysqli::escape_string(getbahaname($id)) . "',`EMAIL` =  '" . mysqli::escape_string($email) . "',`HASHED_MAIL` =  '" . mysqli::escape_string($hashmail) . "',`MODIFY_IP` =  '" . mysqli::escape_string($ip) . "',`MODIFY_TIME` = '" . mysqli::escape_string($Time) . "' WHERE `BAHA_ID`='" . mysqli::escape_string($id) . "'";
...
if($mode=="baha"){
    ...
    if($id!=""&&$hash!=""){
        $sql = "SELECT `BAHA_ID`,`verifycomplete`,`EXPIRE_TIME` FROM `verify` WHERE `BAHA_HASH`='" . mysqli::escape_string($hash) . "' order by `ID` desc LIMIT 1";
        $result = @mysqli_query($con, $sql);
        $data = $result->fetch_array();

        $sql = "SELECT `verifycomplete` FROM `verify` WHERE `BAHA_ID`='" . mysqli::escape_string($id) . "' order by `ID` desc LIMIT 1";
        $result = @mysqli_query($con, $sql);
        $data2 = $result->fetch_array();

        $left_time = strtotime($data[2]) - strtotime($Time);
        if($data[0]==$id&&$data[1]==0&&$left_time>=0){
            $sql = "UPDATE `verify` SET  `verifycomplete` =  '1' WHERE  `BAHA_HASH`='" . mysqli::escape_string($hash) . "' order by `ID` desc LIMIT 1";
            mysqli_query($con, $sql);
            if(firstlogin($id)){
                $sql = "INSERT INTO `list` (`ID`, `BAHA_ID`, `BAHA_NAME`, `EMAIL`, `HASHED_MAIL`, `REGISTER_TIME`, `REGISTER_IP`, `MODIFY_TIME`, `MODIFY_IP`) VALUES (NULL, '" . mysqli::escape_string($id) . "','" . mysqli::escape_string(getbahaname($id)) . "', '', '', '" . mysqli::escape_string($Time) . "', '" . mysqli::escape_string($IP) . "', '0000-00-00 00:00:00', '0.0.0.0')";
                mysqli_query($con, $sql);
            }
    ...
    if($id!=""&&$email!=""&&$hash!=""){
        ...
        if($data[0]==$id&&$data[1]==$email&&$data[2]==0&&$left_time>=0){
            $sql = "UPDATE `emailverify` SET  `verifycomplete` =  '1' WHERE  `EMAIL_HASH`='" . mysqli::escape_string($hash) . "' order by `ID` desc LIMIT 1";
            @mysqli_query($con, $sql);
...
{
    $con = @mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME) or trigger_error('Could not connect to MySQL: ' . mysqli_connect_error());
    $sql = "SELECT `ID` FROM `list` WHERE `BAHA_ID`='" . mysqli::escape_string($id) . "'";
    $result = @mysqli_query($con, $sql);
    $data = $result->fetch_array();
    if($data[0]==""){
        return true;
